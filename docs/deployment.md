...
4. 107 保持旧 raw 原始字节不变；现有 v0.3.7 及以上 runtime 可继续拉取和处理，新装使用
   v0.3.10 SIF；
...
v0.3.10 启动后必须确认日志没有持续的 `subscription audit`、`targeted subscription recovery incomplete`
或 writer failure，
...
新装时 107 与 Vultr 统一使用 v0.3.10 release。两端 ACK 状态与故障恢复验收见
