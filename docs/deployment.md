...
4. 107 保持旧 raw 原始字节不变，把旧 runtime 与控制证据移入只读 archive，再安装 v0.3.9 SIF；
...
v0.3.9 启动后必须确认日志没有持续的 `subscription audit`、`targeted subscription recovery incomplete`
或 writer failure，
...
新装时 107 与 Vultr 统一使用 v0.3.9 release。两端 ACK 状态与故障恢复验收见
...
从 v0.3.8 升级 v0.3.9 只更新 Vultr edge image、部署脚本、snapshot interval `0.75` 和
snapshot concurrency `4`。不得 clean start，也不得修改正式 60 币、raw、ready、ACK、universe、
gap、lease 或 formal-start。v0.3.9 不改变 central 合同，107 不需要升级。
