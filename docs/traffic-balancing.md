...
运行期间不持续追逐分钟级波动。采集器只在每天 UTC 日切评估一次：少于 24 个完整块时不自动
再均衡；证据完整但目标分片未变化时不发送任何订阅命令。需要再均衡时，用最近 24 个块的逐币峰值
一次计算 4 条 route 的确定性目标分片。

这意味着日内短时冲击可能暂时形成热点 route。该延迟是完整性约束：冲击期间立即跨 route 搬币会
增加订阅交接和 snapshot 重锚。小时块保存分钟峰值，最近 24 块再取逐币最大值，所以冲击会进入
下一次日切证据而不会被小时平均稀释。
...
成员轮换和流量再均衡共用全局两阶段交接。第一阶段在所有目标 route 上增加订阅，并分别等待控制
ACK、所需 L2 snapshot 和新增币的第一条 `bookTicker`、`depth` 新事件；只有所有目标 route 都就绪
